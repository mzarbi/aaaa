...
from extensions.mysql_backend import MySqlStorage
from extensions.redis_backend import RedisStorage
from task_queue import signals as S
...
from task_queue.exceptions import TaskQueueException
from task_queue.exceptions import RetryTask
from task_queue.exceptions import TaskException
from task_queue.exceptions import TaskLockedException
from task_queue.registry import Registry
from task_queue.serializer import Serializer
from task_queue.storage import FileStorage
from task_queue.storage import MemoryStorage
from task_queue.storage import SqliteStorage
from task_queue.utils import Error
from task_queue.utils import normalize_expire_time
from task_queue.utils import normalize_time
from task_queue.utils import reraise_as
from task_queue.utils import string_type
from task_queue.utils import time_clock
...
class TaskQueue(object):
    """
    TaskQueue executes tasks by exposing function decorators that cause the function
    ...
    def __init__(self, name='taskqueue', results=True, store_none=False, utc=True,
        ...
        if invalid:
            warnings.warn('the following taskqueue initialization arguments are no '
    ...
    def get_storage(self, **kwargs):
        if self.storage_class is None:
            warnings.warn('storage_class not specified when initializing '
                          'taskqueue, will default to RedisStorage.')
...
class TaskWrapper(object):
    ...
    def __init__(self, taskqueue, func, retries=None, retry_delay=None,
        ...
        # Dynamically create task class and register with taskqueue instance.
        self.task_class = self.create_task(func, context, name, **settings)
        self.taskqueue._registry.register(self.task_class)

    def unregister(self):
        return self.taskqueue._registry.unregister(self.task_class)
    ...
    def is_revoked(self, timestamp=None, peek=True):
        return self.taskqueue.is_revoked(self.task_class, timestamp, peek)

    def revoke(self, revoke_until=None, revoke_once=False):
        self.taskqueue.revoke_all(self.task_class, revoke_until, revoke_once)

    def restore(self):
        return self.taskqueue.restore_all(self.task_class)
    ...
    def schedule(self, args=None, kwargs=None, eta=None, delay=None,
        ...
        eta = normalize_time(eta, delay, self.taskqueue.utc)
        task = self.task_class(
            ...
            priority=priority,
            expires=expires)
        return self.taskqueue.enqueue(task)

    def _apply(self, it):
        return [self.s(*(i if isinstance(i, tuple) else (i,))) for i in it]

    def map(self, it):
        return ResultGroup([self.taskqueue.enqueue(t) for t in self._apply(it)])

    def __call__(self, *args, **kwargs):
        return self.taskqueue.enqueue(self.s(*args, **kwargs))
...
class TaskLock(object):
    ...
    details see :py:meth:`taskqueue.lock_task`.
    """

    def __init__(self, taskqueue, name):
        self._taskqueue = taskqueue
        self._name = name
        self._key = '%s.lock.%s' % (self._taskqueue.name, self._name)
        self._taskqueue._locks.add(self._key)

    def is_locked(self):
        return self._taskqueue.storage.has_data_for_key(self._key)
    ...
    def __enter__(self):
        if not self._taskqueue.put_if_empty(self._key, '1'):
            raise TaskLockedException('unable to set lock: %s' % self._name)

    def __exit__(self, exc_type, exc_val, exc_tb):
        self._taskqueue.delete(self._key)

    def clear(self):
        return self._taskqueue.delete(self._key)
...
class Result(object):
    ...
    def __init__(self, taskqueue, task):
        self.taskqueue = taskqueue
        self.task = task
    ...
    def _get(self, preserve=False):
        ...
        if self._result is EmptyData:
            res = self.taskqueue.get_raw(task_id, peek=preserve)

            if res is not EmptyData:
                self._result = self.taskqueue.serializer.deserialize(res)
                return self._result
            else:
                return res
        else:
            ...
            while self._result is EmptyData:
                if timeout and time_clock() - start >= timeout:
                    if revoke_on_timeout:
                        self.revoke()
                    raise TaskQueueException('timed out waiting for result')
                if delay > max_delay:
                    delay = max_delay
    ...
    def is_revoked(self):
        return self.taskqueue.is_revoked(self.task, peek=True)

    def revoke(self, revoke_once=True):
        self.taskqueue.revoke(self.task, revoke_once=revoke_once)

    def restore(self):
        return self.taskqueue.restore(self.task)
    ...
    */n = run every "n" times, i.e. hours='*/4' == 0, 4, 8, 12, 16, 20
    ...
    for (date_str, value, acceptable) in validation:
        ...
        for piece in value.split(','):
            ...
            # Older versions of taskqueue would, at this point, ignore the unmatched piece.
            if strict:
                raise ValueError('%s is not a valid input' % piece)
